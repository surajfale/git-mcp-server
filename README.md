...
Railway is a modern platform-as-a-service that makes it easy to deploy the Git Commit MCP Server as a remote service accessible over the network. This allows you to use the server from any MCP client without local installation.

### Why Deploy to Railway?

- **Remote Access**: Use the server from any device or MCP client
- **Persistent Storage**: Cloned repositories persist across deployments
- **Automatic HTTPS**: Built-in SSL certificates and custom domain support
- **Easy Deployment**: Deploy directly from GitHub with automatic CI/CD
- **Affordable**: Hobby plan starts at $5/month with generous limits
...
3. **Add Persistent Volume**:
   ...
   - Scroll to "Volumes" section → Click "New Volume"
   - Set mount path to `/data` and size to 2-5 GB
...
5. **Deploy**: Railway automatically builds and deploys your service

6. **Get Your URL**: Railway provides a URL like `https://your-app.railway.app`

### Railway Plan Requirements

The server works with Railway's **Hobby plan** ($5/month):
...
After deploying to Railway, configure your MCP client to use the remote server:
...
Replace:
- `your-app.railway.app` with your Railway-provided URL
- `your-mcp-auth-token` with the token you set in `MCP_AUTH_TOKEN`

### Generating a Secure Auth Token

Use one of these methods to generate a secure authentication token:
...
For complete deployment and API information:

- **[Railway Deployment Guide](docs/railway-deployment.md)** - Step-by-step deployment walkthrough, volume configuration, environment variables, custom domains, and troubleshooting
- **[API Reference](docs/api-reference.md)** - Complete HTTP API documentation with request/response examples, authentication, endpoints, and client examples
- **[Railway Troubleshooting](docs/railway-troubleshooting.md)** - Common Railway deployment issues and solutions
- **[Authentication Guide](docs/authentication.md)** - Security configuration and best practices

### Railway Deployment Button

[![Deploy on Railway](https://railway.app/button.svg)](https://railway.app/template/git-commit-mcp)

Click the button above to deploy with pre-configured settings (coming soon).
